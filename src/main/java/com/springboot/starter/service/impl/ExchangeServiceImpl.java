package com.springboot.starter.service.impl;

import java.io.IOException;
import java.text.DecimalFormat;
import java.util.HashMap;

import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import org.springframework.stereotype.Service;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.JsonMappingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.springboot.starter.model.dto.ExchangeRatesLatestDTO;
import com.springboot.starter.model.dto.ExchangeRequestDTO;
import com.springboot.starter.model.dto.ExchangeResponseDTO;
import com.springboot.starter.service.ExchangeService;

import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;

@Service
public class ExchangeServiceImpl implements ExchangeService{
	
    private static final Logger log = LogManager.getLogger(ExchangeServiceImpl.class);
    
    private static final DecimalFormat df = new DecimalFormat("0.00");

	@Override
	public ExchangeResponseDTO getExchangeRate(ExchangeRequestDTO requestDto) throws JsonMappingException, JsonProcessingException {
		
		ExchangeResponseDTO responseDto = new ExchangeResponseDTO();
		String jsonData = "";
		
		OkHttpClient client = new OkHttpClient().newBuilder()
				  .build();
				Request request = new Request.Builder()
				  .url("http://api.exchangeratesapi.io/v1/latest?access_key=3c75fafa174a777a4de251e0a77b24d6")
				  .method("GET", null)
				  .build();
		try {
			Response response = client.newCall(request).execute();
			jsonData = response.body().string();
		} catch (IOException e) {
			jsonData = "{\"success\":true,\"timestamp\":1650046924,\"base\":\"EUR\",\"date\":\"2022-04-15\",\"rates\":{\"USD\":1.080843,\"AUD\":1.46164,\"CAD\":1.362673,\"PLN\":4.63,\"MXN\":21.549736}}";
		}
		
		ObjectMapper om = new ObjectMapper();
		om.configure(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY, true);
		ExchangeRatesLatestDTO exchangeRateDTO;
		exchangeRateDTO = om.readValue(jsonData, ExchangeRatesLatestDTO.class);
		exchangeRateDTO.getRates();
		
		Double inputRate = null;
		Double outputRate = null;
		
		for (HashMap<String, Double> map : exchangeRateDTO.getRates()) {
			for (String key : map.keySet()) {					
				if (key.equals(requestDto.getCurrencyCodeInput())) {
					inputRate = map.get(key);
				}
				if (key.equals(requestDto.getCurrencyCodeOutput())) {
					outputRate = map.get(key);
				}				
			}			
		}
		
		responseDto.setExchangeRate(outputRate / inputRate);

		return responseDto;
	}

	@Override
	public ExchangeResponseDTO buyCurrency(ExchangeRequestDTO requestDto) throws JsonMappingException, JsonProcessingException {
		ExchangeResponseDTO responseDto = new ExchangeResponseDTO();
		String jsonData = "";
		
		OkHttpClient client = new OkHttpClient().newBuilder()
				  .build();
				Request request = new Request.Builder()
				  .url("http://api.exchangeratesapi.io/v1/latest?access_key=3c75fafa174a777a4de251e0a77b24d6")
				  .method("GET", null)
				  .build();
		try {
			Response response = client.newCall(request).execute();
			jsonData = response.body().string();
		} catch (IOException e) {
			jsonData = "{\"success\":true,\"timestamp\":1650046924,\"base\":\"EUR\",\"date\":\"2022-04-15\",\"rates\":{\"USD\":1.080843,\"AUD\":1.46164,\"CAD\":1.362673,\"PLN\":4.63,\"MXN\":21.549736}}";
		}
		
		ObjectMapper om = new ObjectMapper();
		om.configure(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY, true);
		ExchangeRatesLatestDTO exchangeRateDTO;
		exchangeRateDTO = om.readValue(jsonData, ExchangeRatesLatestDTO.class);
		exchangeRateDTO.getRates();
		
		Double inputRate = null;
		Double outputRate = null;
		
		for (HashMap<String, Double> map : exchangeRateDTO.getRates()) {
			for (String key : map.keySet()) {					
				if (key.equals(requestDto.getCurrencyCodeInput())) {
					inputRate = map.get(key);
				}
				if (key.equals(requestDto.getCurrencyCodeOutput())) {
					outputRate = map.get(key);
				}
				
			}			
		}
		
		String brought = df.format((outputRate / inputRate) * requestDto.getAmountInput());
		String message = String.format("Brought %s %s using %s %s. Transaction is saved" , brought ,
				requestDto.getCurrencyCodeOutput(), requestDto.getAmountInput() , requestDto.currencyCodeInput);
		responseDto.setMessage(message);

		return responseDto;
	}

	@Override
	public ExchangeResponseDTO sellCurrency(ExchangeRequestDTO requestDto) throws JsonMappingException, JsonProcessingException {
		ExchangeResponseDTO responseDto = new ExchangeResponseDTO();
		String jsonData = "";
		
		OkHttpClient client = new OkHttpClient().newBuilder()
				  .build();
				Request request = new Request.Builder()
				  .url("http://api.exchangeratesapi.io/v1/latest?access_key=3c75fafa174a777a4de251e0a77b24d6")
				  .method("GET", null)
				  .build();
		try {
			Response response = client.newCall(request).execute();
			jsonData = response.body().string();
		} catch (IOException e) {
			jsonData = "{\"success\":true,\"timestamp\":1650046924,\"base\":\"EUR\",\"date\":\"2022-04-15\",\"rates\":{\"USD\":1.080843,\"AUD\":1.46164,\"CAD\":1.362673,\"PLN\":4.63,\"MXN\":21.549736}}";
		}
		
		ObjectMapper om = new ObjectMapper();
		om.configure(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY, true);
		ExchangeRatesLatestDTO exchangeRateDTO;
		exchangeRateDTO = om.readValue(jsonData, ExchangeRatesLatestDTO.class);
		exchangeRateDTO.getRates();
		
		Double inputRate = null;
		Double outputRate = null;
		
		for (HashMap<String, Double> map : exchangeRateDTO.getRates()) {
			for (String key : map.keySet()) {					
				if (key.equals(requestDto.getCurrencyCodeInput())) {
					inputRate = map.get(key);
				}
				if (key.equals(requestDto.getCurrencyCodeOutput())) {
					outputRate = map.get(key);
				}
				
			}			
		}
		
		String sold = df.format((outputRate / inputRate) * requestDto.getAmountInput());
		String message = String.format("Sold %s %s into %s %s. Transaction is saved", requestDto.getAmountInput() , requestDto.currencyCodeInput , sold ,
				requestDto.getCurrencyCodeOutput());
		responseDto.setMessage(message);

		return responseDto;
	}





}
